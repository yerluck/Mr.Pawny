...
#include "AutoLight.cginc"
    ...
    half4 frag(v2f o) : COLOR{
        // half4 paint = tex2D(_MainTex, o.uv0); // main texture
        // half4 main_color = (tex2D(_PaintMap, o.uv1)); // painted on texture
        half4 main_color = tex2D(_MainTex, o.uv0); // main texture
        half4 paint = (tex2D(_PaintMap, o.uv1)); // painted on texture
        main_color *= paint;
        // main_color -= lerp(main_color, paint, 1.0f); // add paint to main;
        return main_color;
        // return half4(main_color.r, main_color.g, main_color.b, 0.1f);
    }
        ENDCG
    }
    }
